...
> ⚠️ RRC 示例为简化示意，实际部署请参照 TS 38.331 | 规范版本：Rel-17 | 
...
- [十三、Rel-17 URLLC 增强](#十三rel-17-urllc-增强)
  - [13.1 DCI 0_2：Ultra-Compact UL 调度（Rel-17）](#131-dci-0_2ultra-compact-ul-调度rel-17)
...
- [十六、新增关键技术（Rel-16/17/18 前瞻）](#十六新增关键技术rel-161718-前瞻)
...
[^Rel-15]: **Rel-15 基线，Rel-16/17 增强** — 3GPP TS 22.104 定义了 URLLC 的典型部署场景和量化指标，不同场景对延迟/可靠性的要求差异显著。

#### 典型场景量化需求（TS 22.104 Clause 5）

|| 场景 | 端到端延迟 | 可靠性 | 周期/包长 | 典型用例 |
|------|---------|---------|---------|---------|
| **工厂自动化** | ≤ 0.5~1 ms | 99.999% | 1~2 ms 周期 | 运动控制、机器人协作 |
| **配电自动化** | ≤ 4~15 ms | 99.9% | GOOSE/SMV 周期 | 继电保护、配网监控 |
| **离散自动化** | ≤ 2~10 ms | 99.99% | 2~10 ms 周期 | PLC 通信、视觉检测 |
| **运动控制** | ≤ 0.25~1 ms | 99.999% | 0.5~1 ms 周期 | 伺服电机控制 |
| **远程控制** | ≤ 25~50 ms | 99.99% | 非周期/事件驱动 | 远程驾驶、重型机械 |
| **触觉交互** | ≤ 0.5~1 ms | 99.999% | ≤ 1 ms 周期 | 远程手术、触觉反馈 |
| **智能电网 V2X** | ≤ 3~10 ms | 99.9% | 10~100 ms 周期 | 充电设施管理 |
| **自动驾驶** | ≤ 3~10 ms | 99.99% | 10~100 ms 周期 | 感知共享、协同驾驶 |

> 📖 **规范来源**：TS 22.104 Clause 5（Service requirements for URLLC）；TS 23.501 Clause 5.6.6（Network-slice-specific requirements for URLLC）
...
运动控制（1 ms, 99.999%）：
  - PHY:   60kHz SCS + Type B 2符号 mini-slot
  - MAC:   CG Type 1 (periodicity = sym1x14 @ 60kHz)
  - RLC:   UM (无 ARQ 重传延迟)
  - PDCP:  Duplication (双路径)
  - 预估延迟: ~167 μs（2符号@60kHz + 处理）

工厂自动化（1 ms, 99.999%）：
  - PHY:   60kHz SCS + Type B 4符号
  - MAC:   CG Type 1 + LCP Restrictions
  - RLC:   UM
  - PDCP:  Duplication + t-Reordering
  - 预估延迟: ~333 μs（4符号@60kHz + RTT）

配电保护（4 ms, 99.9%）：
  - PHY:   15/30kHz SCS + Type A 1 slot
  - MAC:   CG Type 2（PDCCH 激活）
  - RLC:   UM
  - 预估延迟: ~1~2 ms（slot aggregation 可选）

远程驾驶（25 ms, 99.99%）：
  - PHY:   15kHz SCS + Slot Aggregation (n4~n8)
  - MAC:   Dynamic Grant + CBG retransmission
  - RLC:   AM (可容忍 1~2 次重传)
  - PDCP:  无需 Duplication
  - 预估延迟: ~10~15 ms（动态调度）
```
...
[^Rel-15]: **Rel-15 引入** — Mini-slot Type B 调度是最早的 URLLC 低延迟机制，支持 1~14 符号的任意起始位置和持续时长，通过 Type B mappingType 实现亚毫秒级调度。
...
**URLLC 优势：**
- 端到端传输时延：从 1 ms（15kHz + Type A）降至 ~167 μs（60kHz + Type B 2符号 = 2 × 16.67 μs + 处理开销 ≈ 167 μs）
...
[^Rel-15]: **Rel-15 引入** — Slot Aggregation 最初引入（max 8 slots）。**Rel-16** 扩展到 max 32 slots，并引入 inter-slot DMRS Bundling 用于增强信道估计质量。
...
1. **可靠性提升**：N 次 slot 重复传输，BLER 改善约 N 倍（Chase Combining 带来 ~10log₁₀N dB SNR 增益）
2. **覆盖增强**：小区边缘 UE 可获得更高可靠性
3. **可靠性-延迟权衡**：可通过减少聚合 Slots 数换取更低的延迟

| 聚合 Slots 数 | 理论 BLER（单次 10⁻³） | 可靠性提升 | 附加延迟 |
|---------------|----------------------|-----------|---------|
| 1（无聚合） | 10⁻³ | 基准 | 0 |
| 2 | ~10⁻⁶（至少1次成功） | ~1000× | +1 slot |
| 8 | ~10⁻²⁴ | ~10²¹× | +7 slots |
| 32 | ~10⁻³⁰ | ~10²⁷× | +31 slots |

> ⚠️ 注：Slot Aggregation 的增益主要来自 **Chase Combining**（每次传输失败后重传，UE 软合并历史软信息），相当于等效 SNR 提升 ~10log₁₀N dB（N=聚合数）。实际增益受信道相关性、移动速度、多径效应等因素影响，需结合仿真或实测评估。

---

### 2.3 LDPC Base Graph 2 / 低码率

[^Rel-15]: **Rel-15 引入** — LDPC Base Graph 2 从 Rel-15 开始支持，专为小数据块（< 292 bits）和低码率（< 1/5）场景设计，提供约 3 dB 额外编码增益。**Rel-17** 将 UL BG2 引入（此前仅 DL 支持）。
...
[^Rel-15]: **Rel-15 引入** — Type A/Type B 时域资源分配格式从 Rel-15 开始定义，Type B 的灵活性来自 SLIV（起始符号 + 持续长度），K0/K2 在规范中始终以 slots 为单位。
...
[^Rel-15]: **Rel-15 引入** — Processing Capability I/II 从 Rel-15 开始定义，URLLC 使用 Capability I 实现最短处理时间。**Rel-16** 新增 `urllc-Mode` IE 以结构化上报 URLLC 能力。
...
[^Rel-15]: **Rel-15 引入** — 5 个 Numerology（μ=0~4，SCS 15kHz~240kHz）从 Rel-15 开始定义，支持不同场景需求。
...
[^Rel-15]: **Rel-15 引入** — DCI 0_0/0_1/1_0/1_1 从 Rel-15 开始定义，CBG retransmission 在 Rel-15 的 DCI 1_1/0_1 中引入。**Rel-17** 新增 DCI 0_2（超紧凑 UL 调度，约 30 bits）。
...
[^Rel-15]: **Rel-15 引入** — CQI Table 3（10⁻⁵ BLER）从 Rel-15 开始定义，专为 URLLC 场景设计。
...
[^Rel-15]: **Rel-15 引入** — CG Type 1（RRC 配置，无 PDCCH 激活）从 Rel-15 开始定义。**Rel-16** 新增 repK 重复传输、cg-RetransmissionTimer、configuredGrantTimer 等增强。
...
[^Rel-15]: **Rel-15 引入** — CG Type 2（RRC + PDCCH 激活）与 Type 1 一同在 Rel-15 定义。**Rel-16** 新增 Multiple Entry CG Confirmation（32 CG-fields Octet bitmap）。
...
[^Rel-16]: **Rel-16 引入** — cg-RetransmissionTimer 从 Rel-16 开始定义，用于 LBT 失败后的自主重传控制，适用于 Unlicensed 频段 URLLC。
...
[^Rel-15]: **Rel-15 引入** — LCP Restrictions 是最早的标准化的 URLLC 资源隔离机制，允许 RRC 配置逻辑信道的资源限制（allowedSCS-List、maxPUSCH-Duration、configuredGrantType1Allowed、allowedServingCells）。**Rel-16** 新增 allowedCG-List-r16（按 CG 索引精细控制）和 allowedPHY-PriorityIndex-r16（PHY 优先级控制）。**Rel-17** 新增 allowedHARQ-mode-r17（允许的 UL HARQ 模式）。
...
| 参数 | 说明 | URLLC 用途 | 引入 Release |
|------|------|-----------|------------|
| **allowedSCS-List** | 允许的 SCS 列表 | URLLC 需要高 SCS（60kHz+） | Rel-15 |
| **maxPUSCH-Duration** | 最大 PUSCH 持续时长 | URLLC 需要短传输（≤4 符号） | Rel-15 |
| **configuredGrantType1Allowed** | 是否允许 Type 1 CG | URLLC 可绑定 grant-free 资源 | Rel-15 |
| **allowedServingCells** | 允许的 Serving Cells | URLLC 可能只在特定小区 | Rel-15 |
| **allowedCG-List-r16** | 允许的 CG 配置索引列表（MAC 层索引） | URLLC 绑定特定 CG，比 R15 更细粒度 | Rel-16 |
| **allowedPHY-PriorityIndex-r16** | 允许的 PHY 优先级索引（p0=low/p1=high） | URLLC 高优先级动态调度 | Rel-16 |
| **allowedHARQ-mode-r17** | 允许的 UL HARQ 模式（harqModeA/harqModeB） | URLLC 可选择特定 HARQ 模式 | Rel-17 |
...
For each UL grant:
  For each logical channel j:
    IF ALL of the following are true:
      ...
      - allowedCG-List-r16 includes CG index (Rel-16), AND
      - allowedPHY-PriorityIndex-r16 includes priority (Rel-16), AND
      - allowedHARQ-mode-r17 includes HARQ mode (Rel-17)
    THEN:
...
[^Rel-16]: **Rel-16 引入** — lch-basedPrioritization 算法从 Rel-16 开始标准化，用于处理多个重叠上行授权时的优先级判定。
...
[^Rel-15]: **Rel-15 引入** — SR（Scheduling Request）机制从 Rel-15 开始定义，URLLC 可配置更短的 sr-ProhibitTimer 和专用 SR 配置。**Rel-16** 新增 SR 在 Scell 上的配置支持。
...
[^Rel-15]: **Rel-15 引入** — BSR 机制从 Rel-15 开始定义。**Rel-16** 新增 logicalChannelSR-DelayTimer（BSR batching）和 Extended BSR。**Rel-17** 新增 CG-SDT 相关的 BSR 优化。
...
[^Rel-15]: **Rel-15 引入** — RLC UM（Unacknowledged Mode）避免了 ARQ 重传导致的延迟不确定性，是 URLLC 的首选 RLC 模式。t-Reassembly 定时器（DL）为 Rel-15 标准配置。
...
[^Rel-15]: **Rel-15 引入** — t-Reassembly 定时器从 Rel-15 开始定义，用于 RLC UM 模式下处理乱序 PDCP PDUs 的重组。
...
[^Rel-15]: **Rel-15 引入** — RLC UM 模式及 SN 字段长度选择（6 bits/12 bits）从 Rel-15 开始定义。
...
[^Rel-15]: **Rel-15 基础，Rel-16/17 扩展** — PDCP Duplication、t-Reordering 为 Rel-15 原生；Rel-16 新增 moreThanOneRLC（支持双流）；Rel-17 将 RLC Entity 上限扩展至 4 个（TS 38.331 Clause 6.3.2）。
...
有 Duplication（双路径）：
  - BLER: ~10⁻⁵（单路径 10⁻³ 时，双路径分集至少成功一次的等效 BLER ≈ 10⁻³ × 10⁻³ = 10⁻⁶，提升约 1000 倍）
...
[^Rel-15]: **Rel-15 引入** — PDCP t-Reordering 定时器从 Rel-15 开始定义，用于处理来自不同 RLC 实体或 HARQ 重传的 PDCP PDU 乱序到达。
...
[^Rel-15]: **Rel-15 引入** — PDCP Status Report 从 Rel-15 开始定义，用于 AM 模式下的选择性重传和丢包检测。
...
[^Rel-15]: **Rel-15 基础** — HARQ-ACK、UCI on PUSCH、CBG-based Retransmission 均为 Rel-15 原生特性。Rel-16 的 sSCell 切换机制进一步优化了 TDD 系统中的 HARQ-ACK 时序。

### 6.1 紧凑型 HARQ-ACK 反馈

[^Rel-15]: **Rel-15 原生** — UCI on PUSCH 的复用机制和 HARQ-ACK 紧凑格式在 Rel-15 TS 38.212/213 中定义，Rel-16/17 无重大架构变化，仅扩展了 UCI 类型（CG-UCI）。

#### 技术原理

URLLC 需要快速 HARQ-ACK 反馈以降低延迟。通过紧凑的 UCI（Uplink Control Information）格式减少 ACK 开销。

> 📖 **规范来源**：TS 38.212 Clause 9.2.5（UCI on PUSCH 编码率计算）、TS 38.213 Clause 9.2.5（HARQ-ACK on PUSCH）

#### UCI 编码率与 betaOffsets 参数

UCI on PUSCH 的资源配置由 `betaOffsets` 参数控制（TS 38.212 Clause 9.2.5）：

```
betaOffsets 参数（CG-UCI-OnPUSCH / UCI-OnPUSCH-Setup）：

β-offset 含义：UCI 相对于 PUSCH 数据的编码率偏移
  - betaOffsets = semi-static_beta_offsets（配置集合）
  - 不同 UCI 类型的编码率目标：
      HARQ-ACK:  target coding rate ≈ PUSCH coding rate × 2^betaOffset
      CSI:        target coding rate ≈ PUSCH coding rate × 2^(betaOffset - 1)

UCI on PUSCH 的资源分配（TS 38.212 Clause 9.2.5）：

1. HARQ-ACK 编码率上界：≤ 0.25（4 info bits @ QPSK, R=1/3 → 12 coded bits）
   - 目标：2 bits HARQ-ACK 在 12~24 coded bits 上传输
   - betaOffset 配置：使编码率不超过 0.25

2. UCI 资源计算：
   O_UCI = UCI info bits（含 CRC）
   Q'UCI = UCI 编码后的实数符号数
   Q'rx = PUSCH 编码后的实数符号数
   β-offset 决定 Q'UCI / Q'rx 的比例

实际配置示例：
  UCI-OnPUSCH-Setup ::= SEQUENCE {
      betaOffsets               CHOICE {
          dynamic               NULL,
          semiStatic            BetaOffsets
      },
      scalingFactor             INTEGER (1..4)   -- UCI vs PUSCH 资源比例
  }
  BetaOffsets ::= SEQUENCE {
      betaOffsetACK-Index       INTEGER (0..31),
      betaOffsetCSI-Part1-Index INTEGER (0..31),
...
```
PUSCH 上可以复用以下 UCI（TS 38.212 Clause 9.2.5）：
- HARQ-ACK（1~2 bits per TB）
- CSI Part 1 + CSI Part 2（信道状态信息）
- PTRS phase hint（相位跟踪参考信号）
- CG-UCI（Configured Grant 状态）

复用优先级（编码率从高到低）：
  1. HARQ-ACK        → 最高优先级，编码率最低（冗余最多）
  2. CG-UCI          → 次高优先级
  3. CSI Part 1      → 中优先级
  4. CSI Part 2      → 最低优先级（可丢弃）

资源映射规则：
  - HARQ-ACK + CSI: UCI 先调制，再与 PUSCH 数据频分复用
  - HARQ-ACK + CG-UCI: HARQ-ACK 优先编码，CG-UCI 占用剩余资源
```

#### Compact HARQ-ACK 格式

```
HARQ-ACK 反馈格式（TS 38.213 Clause 9.1）：

PUCCH Format 0（1~2 bits，最紧凑）：
  - 序列基序列（sequence-based）：纯序列无调制
  - 时域：1~2 符号
  - 频域：1 PRB
  - 优势：最小开销，极低延迟
...
TS 38.213 定义了 HARQ-ACK 码率：
  - 小于等于 2 bits: use PUCCH Format 0/1
  - 大于 2 bits: use PUCCH Format 2
```

### 6.2 PUCCH Cell Switching（sSCell）

[^Rel-16]: **Rel-16 引入** — sSCell（Secondary Cell in switching state）允许 UE 在 TDD 系统中切换到上行小区传输 PUCCH，避免等待 PCell 的上行子帧。
...
[^Rel-15]: **Rel-15 引入** — UCI on PUSCH 复用机制从 Rel-15 开始定义。**Rel-16** 新增 CG-UCI（Configured Grant UCI），用于 CG 状态指示。
...
[^Rel-15]: **Rel-15 引入** — CBG（Code Block Group） retransmission 支持按 CBG 粒度重传，而非整个 TB，是 URLLC 可靠性增强的重要机制。
...
[^Rel-15]: **Rel-15 基础，Rel-16 扩展** — BWP/SR/LogicalChannelConfig 为 Rel-15 原生；Rel-16 新增 allowedPHY-PriorityIndex（§13.6）、SchedulingRequestConfig 扩展、sSCell（见§6.2）。

### 7.1 BWP 配置与 URLLC

[^Rel-15]: **Rel-15 引入** — BWP（Bandyidth Part）配置从 Rel-15 开始定义，支持独立 SCS、CP 和 Numerology 的 BWP，用于 URLLC 流量隔离。
...
[^Rel-15]: **Rel-15 引入** — SchedulingRequestConfig 从 Rel-15 开始定义，URLLC 可配置更短的 sr-ProhibitTimer。
...
[^Rel-17]: **Rel-17 引入** — LogicalChannelSR-Config 从 Rel-17 开始定义，支持为每个逻辑信道配置独立的 SR 参数（sr-ProhibitTimer、sr-TransMax 等）。
...
[^Rel-15]: **Rel-15 引入** — UE-NR-Capability 从 Rel-15 开始定义。**Rel-16** 新增 urllc-Mode IE 以结构化上报 URLLC 能力。
...
[^Rel-16]: **Rel-16 引入** — 双 PDU Session 架构、ATSSS（§8.3）、UPF 复制点机制均在 Rel-16 标准化，用于实现端到端路径冗余和流量协同。
...
[^Rel-16]: **Rel-16 引入** — 同一 UPF 下的双 NG-U tunnel 冗余传输从 Rel-16 开始定义，通过不同传输层路径提供端到端冗余。
...
[^Rel-16]: **Rel-16 引入** — ATSSS（Access Traffic Steering, Switching, Splitting）支持多接入路径的流量引导、切换和分裂，提供 5G 与 WLAN 之间的协同可靠性。
...
[^Rel-16]: **Rel-16 引入** — NR-U（NR in Unlicensed Spectrum）通过 LBT/Channel Access 机制在共享频谱上支持 URLLC，包含 CG Retransmission Timer 等 Rel-16 增强。

### 9.1 LBT/Channel Access 机制

[^Rel-16]: **Rel-16 引入** — NR-U 通过 LBT/Channel Access 机制在 Unlicensed 共享频谱上支持 URLLC，包含 Cat 4 LBT、Channel Access Priority Class 等机制。
...
[^Rel-16]: **Rel-16 新增** — NR-U 的 CG Retransmission Timer 通过 `configuredGrantTimer` 字段配置（TS 38.331 Clause 6.3.2），用于 URLLC 在 Unlicensed 频段的自适应重传。
...
ConfiguredGrantConfig ::= SEQUENCE {
  ...
  configuredGrantTimer           INTEGER (1..64)   OPTIONAL,  -- CG 重传定时器
  ...
}
```

```
configuredGrantTimer 值（单位：sf=1 subframe=1ms @ 15kHz 或对应 slot 数）：
  - 1:   最短 timer（~1 ms）
  - 64:  最长 timer（~64 ms）

Timer 选择考虑：
  - 较短: 高可靠性 + 低延迟（URLLC 优先）
  - 较长: 避开与周围 WiFi/其他 NR-U 设备冲突

NR-U 与 Licensed 频段差异：
  - Licensed CG: 无 LBT 开销，timer 偏小
  - NR-U CG: 必须考虑 LBT 失败概率，timer 偏大
  - 推荐 URLLC NR-U: configuredGrantTimer = 4~16
```

### 9.3 Channel Access Priority Class（CAPC）

[^Rel-16]: **Rel-16 标准化** — TS 38.213 Clause 4（Channel Access Procedure）定义了 4 个 CAPC 等级（Priority Class 1~4），每个等级对应不同的竞争窗口（CW）和最大信道占用时间（MCOT），用于 NR-U 的 LBT 机制。

#### CAPC 等级定义

|| Priority Class | 业务类型 | Contention Window (CW) | MCOT | 说明 |
|---|------|---------|----------------------|------|------|
| 1 | 最高 | URLLC/URLLC-eMBB | CW ∈ {3, 4} | 2 ms | 低延迟优先 |
| 2 | 高 | URLLC | CW ∈ {7, 8, 15, 16} | 4 ms | URLLC 中等可靠性 |
| 3 | 中 | eMBB/Video | CW ∈ {15, 16, 31, 32} | 6/10 ms | 平衡可靠性 |
| 4 | 低 | Background/Best Effort | CW ∈ {31, 32, 63, 64, 127, 128, 255, 256} | 10 ms | 高吞吐 |

> 📖 **规范来源**：TS 38.213 Table 4-1 — Channel Access Priority Class mapping

#### NR-U 上行（UL）的 CAPC

UE 在 Unlicensed 频段发送 PUSCH/PUCCH 时，须根据业务的 QoS 需求选择对应的 CAPC 等级：

```
UL CAPC 选择规则（TS 38.213 Clause 4）：
  - 依据 Logical Channel 的 priority 映射到 CAPC
  - URLLC LCH（priority=1~4）→ CAPC 1 或 2
  - eMBB LCH（priority=5~8）→ CAPC 3 或 4

上行 LBT 类型：
  - Cat A（无随机退避）: 用于 fixed-sized grants，由 gNB 分配的资源
  - Cat B（随机退避）: 用于 UE 自主发送，带 CAPC 竞争窗口
```

### 9.4 eLAA（enhanced LAA）上行机制

[^Rel-16]: **Rel-16 引入** — eLAA（enhanced LAA）将 LAA 的下行机制扩展到上行，允许 UE 在 Unlicensed 频段主动发起 LBT 竞争，适用于 URLLC 的 grant-free 场景。

#### eLAA 与 LAA 的区别

|| 维度 | LAA（Rel-13/14） | eLAA（Rel-16） |
|------|------|------------------|----------------|
| 方向 | 仅 DL | DL + UL |
| UL LBT | 不支持 | Cat B 随机退避 |
| URLLC 支持 | 不支持 | 支持 grant-free CG |
| CAPC | 不支持 | 支持 4 等级 CAPC |

#### eLAA UL LBT 流程

```
eLAA UL LBT 流程（TS 38.213 Clause 4.2）：

1. 触发条件：UE 有上行数据需要发送
2. 选择 CAPC：根据 LCH priority 选择对应 Priority Class
3. 执行 Cat B LBT：
   - 在感知信道空闲后，启动随机退避计数器
   - 计数器在 [0, CW] 范围内均匀分布（CW 由 CAPC 决定）
   - 每感知识道空闲一次，计数器减 1
   - 计数器到 0 时开始传输
4. 发送 PUSCH：传输时长不超过 MCOT（由 CAPC 决定）
5. 信道占用结束：UE 释放信道，进入空闲期
```

### 9.5 LBT Cat A / Cat B 与 NR-U DCI

[^Rel-16]: **Rel-16 定义** — TS 38.212 Clause 7.3.1（DCI formats）定义了 NR-U 特有的 DCI 2_0（Slot Format Indicator on unlicensed）和 DCI 0_0/1_0 的 unlicensed 扩展。

#### Cat A（固定资源无退避）

```
Cat A 适用场景（TS 38.213 Clause 4.1）：
  - gNB 通过 PDCCH 明确分配的资源（类似 licensed 调度）
  - 适用于 CG Type 1（rrc-ConfiguredUplinkGrant）的周期性传输
  - 不需要随机退避，信道空闲即可传输

Cat A 条件：
  - DCI 调度的 PUSCH/PDSCH
  - CG Type 1 配置的周期性资源
  - 任何固定位置的 grant-free 资源
```
...
```
Cat B 适用场景：
  - UE 自主触发的传输（buffer status 驱动）
  - CG Type 2 激活后的上行传输
  - 无明确 PDCCH 分配的上行数据

Cat B LBT 参数：
  - 优先级类别由数据业务的 QoS 决定
  - 计数器初值 = random() mod (CW+1)
  - 退避范围由 CAPC 对应的 CW_min 和 CW_max 决定
```

### 9.6 Channel Occupancy Time（COT）共享

[^Rel-16]: **Rel-16 引入** — gNB 获得的信道占用时间（MCOT）可在一定条件下共享给 UE，减少 UE 侧的 LBT 开销，提升 URLLC 可靠性。

#### COT 共享机制

```
NR-U COT 共享（TS 38.213 Clause 4.3）：

gNB 发起 LBT 成功后：
  1. gNB 获得信道占用（最大 MCOT 由 CAPC 决定）
  2. gNB 可在 DCI 中指示 COT 共享（Flag in DCI 0_0/0_1）
  3. 共享窗口内 UE 无需 LBT，直接发送 PUSCH
  4. COT 共享不能跨 gNB，UE 只能使用同一 gNB 的 COT

URLLC 场景的 COT 共享优势：
  - 消除了 CG Type 2 激活后第一次 PUSCH 的 LBT 延迟
  - 在 MCOT 内可实现多次连续传输
```

### 9.7 NR-U 与 WiFi 共存

[^Rel-16]: **Rel-16 共存机制** — NR-U 通过 LBT 和 CAPC 机制与同频段 WiFi 设备共存，CCA（Clear Channel Assessment）检测阈值要求不低于 -72 dBm（WiFi 设备为 -62 dBm），确保公平竞争。

```
NR-U vs WiFi 共存关键参数：

  NR-U CCA 检测门限: ≤ -72 dBm（宽带能量检测）
  WiFi CCA 检测门限: ≤ -62 dBm（窄带能量检测）
  结论: NR-U 对干扰更敏感，相对"礼貌"

NR-U LBT 公平性设计:
  - CW 最小值随 CAPC 降低（高优先级业务 CW 较小）
  - MCOT 有上限（防止单一设备长时间占用）
  - COT 共享机制减少重复 LBT 开销

WiFi 友好型设计（TS 37.213 Clause 7）：
  - 帧结构兼容 WiFi 的 DIFS/SIFS 时隙边界
  - gNB 可配置 "ankar" 信号减少对 WiFi 的干扰
```
...
[^Rel-16]: **Rel-16 引入** — TSC（Time Sensitive Communications）强调确定性通信和等时通信，通过 10ns 粒度时间同步和 TSN Bridge 集成实现工业 IoT 场景的端到端确定性。

### 10.1 TSC 架构

[^Rel-16]: **Rel-16 引入** — TSC（Time Sensitive Communications）从 Rel-16 开始定义，强调确定性通信和等时通信，通过 10ns 粒度时间同步和 TSN Bridge 集成实现工业 IoT 场景的端到端确定性。
...
[^Rel-16]: **Rel-16 引入** — 10ns 粒度时间同步从 Rel-16 开始定义，通过 GPS、PTP 等机制实现，用于 TSC 场景的端到端确定性。
...
```
5G 时间同步精度要求（TS 38.855 / TS 38.300 Clause 16.8）：

  - 时间同步精度: ± 390 ns（相对于 TSN 主时钟）
  - 时间戳分辨率: 10 ns（TS 38.331 timeDomainOffset 粒度）
  - gNB → UE 同步路径: RRC 信令 + MAC CE 校正

TSN Bridge 集成的 5G 时间同步架构：
  ┌─────────────────────────────────────────────────────┐
  │           TSN Master Clock (Grandmaster)            │
  └──────────────────────┬──────────────────────────────┘
                         │  Ethernet / PTP
  ┌──────────────────────▼──────────────────────────────┐
  │     5G System (as TSN Bridge: Bridge Delay ≤ 1 μs)  │
  │                                                      │
  │  Ingress Port (TSN → 5G):  Bridge Delay Compensated │
  │  Egress Port  (5G → UE):  Bridge Delay Compensated   │
  │                                                      │
  │  5G Time = TSN Time + Bridge Delay +传输延迟         │
  │  UE 获得的时间精度: ± (Bridge Delay + 同步误差)      │
  │                    ≤ ± 1.39 μs (TSN Bridge 要求)    │
  └──────────────────────────────────────────────────────┘
```

> 📖 **规范来源**：TS 38.855 Clause 6.2（Network DN settings）；IEEE 802.1AS（Timing and Synchronization）

#### TSN Bridge Delay Compensation

```
TSN Bridge 作为 5G 系统的时间同步机制（TS 38.856）：

Bridge Delay = Ingress Delay + Internal Delay + Egress Delay

典型 Bridge Delay 预算（5G 系统）：
  - Ingress Port (Ethernet side): ~200 ns
  - 5G Internal processing:        ~500 ns
  - Egress Port (UE side):         ~200 ns
  - 总计 Bridge Delay:             ≤ 1 μs（5G TSN Bridge 要求）

UE 侧时间同步精度（端到端）：
  - 同步源: gNB 通过 RRC 配置 tsc-TimeReferenceInfo
  - timeDomainOffset: INTEGER (0..399) × 10 ns = 0 ~ 3.99 μs
  - UE 同步误差: ≤ ± 390 ns（TSN AS 同步精度）
  - 端到端总精度: ≤ ± (390 ns + Bridge Delay + 传输延迟)
```
...
[^Rel-16]: **Rel-16 标准化** — TSC 流量通过 PDCP + RLC UM + CG Type 1 实现周期性、等时性传输，配置的 periodicity 必须与工业控制的周期匹配。

#### TSC 周期性配置

```
TSC 流量周期配置示例（周期性控制 1 ms）：

ConfiguredGrantConfig:
  - periodicity = sym1x14 (1 slot = 1 ms @ 15kHz SCS)
  - repK = n1
  - rrc-ConfiguredUplinkGrant:
      timeDomainOffset = 0
      timeDomainAllocation = 0 (symbol 0 start)
      frequencyDomainAllocation = BIT STRING(SIZE(18))
      mcsAndTBS = ...

RRC 配置周期性与业务周期的匹配：
  业务周期 1 ms  →  periodicity = sym1x14 @ 15kHz
  业务周期 0.5 ms →  periodicity = sym1x14 @ 30kHz（注意 SCS 变化）
  业务周期 0.25 ms → periodicity = sym1x14 @ 60kHz
```

#### TSC 延迟预算分析

```
TSC 端到端延迟预算（工厂自动化场景，周期 1 ms）：

测量点            累计延迟（μs）   说明
──────────────────────────────────────────────
Sensor sampling       50         传感器采样周期
UE → gNB 传输        67         1 slot @ 15kHz
gNB 转发              50         NG-U 转发
UPF → TSN 网络       100        以太网转发
TSN 交换延迟           10        确定性网络交换
──────────────────────────────────────────────
总延迟               ~277 μs    远低于 1 ms 周期

关键延迟控制机制：
  - CG Type 1 消除 PDCCH 调度等待（~ 0 μs DCI 等待）
  - RLC UM 消除 ARQ 重传（无重传延迟）
  - 10 ns 粒度时间戳确保精确周期对齐
```

> 📖 **规范来源**：TS 22.104 Clause 5（URLLC use cases for TSC）；TS 23.501 Clause 5.6.6（TSC QoS Flow）
...
[^Rel-16]: **Rel-16 引入** — Sidelink Inter-UE Coordination 从 Rel-16 开始定义，通过 PC5-RRC 协调 UE 间资源选择，用于 V2X 和 URLLC sidelink 场景。
...
| **Slot Aggregation** | TS 38.214 Clause 6.1.3 | TB repetition（max 8 slots，Rel-16 扩展至 32） |
| **LDPC BG2（DL）** | TS 38.212 Clause 5.2.2 | 低码率编码，~3 dB 增益 |
| **LCP Restrictions（基础字段）** | TS 38.321 Clause 5.4.3.1 | allowedSCS-List/maxPUSCH-Duration/configuredGrantType1Allowed/allowedServingCells |
| **Packet Duplication** | TS 38.323 Clause 5.1.5 | PDCP 复制到多 RLC entity |
| **CQI Table 3（10⁻⁵ BLER）** | TS 38.214 Clause 5.2.2 | URLLC 可靠性专用 CQI 表 |
| **DCI Format 1_1/0_1** | TS 38.212 Clause 7.3 | CBG retransmission, Priority Indicator |
| **UCI on PUSCH 复用** | TS 38.212 Clause 9.2.5 | HARQ-ACK + CSI 频分复用 |
| **Multi-Connectivity (intra)** | TS 38.300 Clause 16.1.6 | PDCP duplication |
| **RLC UM Mode** | TS 38.322 Clause 5.1 | 无 ARQ 重传，URLLC 首选 |
| **PDCP t-Reordering** | TS 38.323 Clause 5.1.2.1 | 乱序 PDU 重组 |
| **PDCP Status Report** | TS 38.323 Clause 5.1.2.2 | AM 模式丢包检测 |
| **BSR（基础）** | TS 38.321 Clause 5.4.5 | Regular/Periodic/Padding BSR |

### R16 特性（Rel-16 增强）

|| 特性 | 规范 | 核心机制 |
||------|------|----------|
| **Slot Aggregation max 32 slots** | TS 38.214 Clause 6.1.3 | 从 8 扩展至 32 slots |
| **Inter-slot DMRS Bundling** | TS 38.214 Clause 6.1.3 | 跨 slots 连续信道估计 |
| **cg-RetransmissionTimer** | TS 38.321 Clause 5.8.2 | LBT 失败后自主重传 |
| **Multiple Entry CG Confirmation** | TS 38.321 Clause 6.1.3.31 | 32 CG-fields Octet bitmap |
| **lch-basedPrioritization** | TS 38.321 Clause 5.4.3.1 | 重叠授权优先级判定 |
| **allowedCG-List-r16** | TS 38.331 Clause 6.3.2 | 按 CG 索引精细控制 |
| **allowedPHY-PriorityIndex-r16** | TS 38.331 Clause 6.3.2 | PHY 优先级控制（p0/p1） |
| **CG Retransmission Timer（SPS）** | TS 38.331 Clause 6.3.2 | Unlicensed 频段 CG 重传 |
| **sSCell Switching** | TS 38.300 Clause 16.1.8 | TDD 系统 PUCCH 切换 |
| **MoreThanOneRLC** | TS 38.331 Clause 6.3.2 | 双 RLC entity 支持 |
| **URRLC-Mode IE** | TS 38.331 Clause 6.3.2 | URLLC 能力结构化上报 |
| **Multiple SPS** | TS 38.331 sps-Config | 多 SPS 配置并发 |
| **Dual UPF Redundancy** | TS 23.501 Clause 5.6.8 | 双 UPF 冗余路径 |
| **ATSSS** | TS 23.501 Clause 5.6.8A | 多接入流量 Steering/Splitting |
| **TSC 架构** | TS 38.300 Clause 16.8 | 确定性通信/等时通信 |
| **10ns 时间同步** | TS 38.300 Clause 16.8 | 工业 IoT 端到端确定性 |
| **NR-U（LBT/Channel Access）** | TS 37.213 / TS 38.300 Clause 16.1.7 | Unlicensed 频段 URLLC |
| **logicalChannelSR-DelayTimer** | TS 38.321 Clause 5.4.5 | BSR batching timer |
...
### 13.2 DCI 0_2：Ultra-Compact UL 调度（Rel-17）

[^Rel-17]: **Rel-17 引入** — DCI 0_2（TS 38.212 Clause 7.3.1.2.1）是最紧凑的 UL 调度格式，通过字段压缩实现 ~30 bits 级别开销，专为高频 URLLC 小包设计。相比 Rel-16 的 DCI 0_0（~70 bits），开销减少约 57%。
...
[^Rel-17]: **Rel-17 引入** — Enhanced Reliability 包括 UL LDPC BG2（上行短 TB 3dB 增益）、Inter-slot DMRS Bundling（跨 slots 信道估计）、4-path Packet Duplication、PDCCH 监听增强等多维度可靠性提升。
...
[^Rel-16]: **Rel-16 引入** — Multiple TCI States 允许 PDCCH 和 PDSCH 配置多个波束/准 co-location 状态，通过 DCI 动态切换或预配置模式实现快速 beam switching，适用于高铁/车载场景。
...
[^Rel-16]: **Rel-16 引入** — `repK`（非 repKARQ-Procedure）字段配置 PUSCH 重复次数（n1/n2/n4/n8），支持无 HARQ-ACK 的重复传输，消除 RTT 延迟，适合极低延迟高可靠性场景。
...
[^Rel-16]: **Rel-16 引入** — CG Type 1 和 Type 2 的共存关系由 Rel-16 标准化，`configuredGrantType1Allowed` 和 `configuredGrantType2Allowed` 两个独立参数控制同一逻辑信道对两种 CG 类型的偏好。
...
[^Rel-16]: **Rel-16 引入** — `allowedPHY-PriorityIndex` 为 Rel-16 在 `LogicalChannelConfig` 中新增字段（TS 38.331 Clause 6.3.2），用于区分 URLLC 和 eMBB 逻辑信道的调度优先级，允许 UE 在同一 PUSCH 上复用不同优先级的上行数据。
...
> 在阅读以下路径图之前，首先需要明确：**数据本身不会自带"URLLC 标签"**，URLLC 的确定性传输能力来自网络侧预先配置的各项机制。UE 和 gNB 按照 RRC 配置的规则处理数据，"碰巧"上了 URLLC 专用资源的即为 URLLC 数据。

#### URLLC 数据的识别链路

```
Application Layer（业务数据）
  ↓
SDAP 根据 5QI 映射到 QoS Flow
  → 5QI=82/83（Rel-16 URLLC 专用）或 5QI=8/9（由 DRB 配置决定）
  ↓
PDCP 根据 DRB 配置路由到对应 RLC 实体
  → 该 DRB 配置了 LogicalChannelConfig（priority/highestPriority）
  ↓
RLC（UM 模式，URLLC 通常无 ARQ 重传）
  ↓
MAC 从各逻辑信道 buffer 按 priority 选取发送
  → LCP Restrictions 决定 grant 能否用于该逻辑信道
  → CG Type 1 资源配置了 periodicity/allowedCG-List
  → UE 在预配置资源上自主传输，无需 SR + PDCCH
```
...
| 层级 | 区分标准 | 说明 |
|------|---------|------|
| **SDAP** | 5QI 值 | 82/83 = URLLC，8/9 = 普通 GBR |
| **PDCP** | DRB 配置 | DRB 的 `pdcp-Config + rlc-Config` 决定处理方式 |
| **RLC** | RLC Mode | URLLC 通常用 UM（无 ARQ，低延迟） |
| **MAC** | 逻辑信道 priority | 数值越小优先级越高 |
| **MAC** | LCP Restrictions | grant 与逻辑信道的匹配过滤 |
| **CG** | allowedCG-List | grant-free 资源池的逻辑信道白名单 |

**三条路径的选择逻辑：**
- 路径 A（CG Type 1）：小数据量、高可靠性、低延迟 —— 无需调度等待
- 路径 B（动态调度）：大数据量、正常可靠性 —— 标准 SR → PDCCH → Grant 流程
- 路径 C（Duplication + Slot Aggregation）：最高可靠性需求 —— 双路径分集 + 多 slot 重复
...
**关键特征**：双路径并行传输，任一路径成功即可交付（PDCP Discard 机制去重）。假设每路径 BLER = 10⁻³，双路径等效 BLER ≈ 10⁻³ × 10⁻³ = 10⁻⁶，可靠性提升约 **1000 倍**。
...
## 十六、新增关键技术（Rel-16/17/18 前瞻）

> ⚠️ **版本说明**：本章涵盖 Rel-16 已标准化特性（DCI 1_2、SPS、PUSCH Repetition Type A/B）与 Rel-18 前瞻方向（AI/ML 调度、XR 增强）。标有 🔒 的小节为 3GPP 已发布规范（TS 38.212/213/331），标有 🔭 的小节为 Rel-18/5G-Advanced 方向性内容（标准可能未完全固化）。
>
> 📖 **规范来源**：DCI 1_2 → TS 38.212 Clause 7.3.1.2.2；SPS → TS 38.321 Clause 5.10.1；PUSCH Repetition → TS 38.331 Clause 8.3；端到端延迟 → TS 38.300 Clause 16.1.1；QoS Monitoring → TS 23.501 Clause 5.7.7；CU/DU 架构 → TS 38.401 Clause 6；Rel-18 → TS 38.300 Clause 9.2.8

### 16.1 DCI 1_2 紧凑型 DL 调度

🔒 **Rel-16 已标准化** — DCI 1_2 是 3GPP Rel-16 引入的紧凑型 DL 调度格式（TS 38.212 Clause 7.3.1.2.2），与 DCI 0_2 配对使用。通过 RIV 编码和字段压缩（DAI 2bit→1bit, CSI request 3bit→2bit），控制信道开销减少约 30%。
...
🔒 **Rel-16 已标准化** — TS 38.321 Clause 5.10.1 定义了 SPS（Semi-Persistent Scheduling），与 UL CG 形成对称的免调度 DL 机制。

#### 16.2.1 SPS 与 CG 的对称性（🔒 Rel-16）
...
SPS-Config ::= SEQUENCE {
    n1PUCCH-AN                  INTEGER (0..65535)   OPTIONAL,  -- HARQ-ACK 反馈资源
    periodicity                 SPS-Periodicity      OPTIONAL,  -- 周期性
    ...  -- other fields
}

SPS-Periodicity ::= ENUMERATED {
    n1, n2, n4, n8, n10, n16, n20, n32,   -- slots
    n64, n128, spare4, spare3, spare2, spare1
}

# 激活后的 SPS 参数由 PDCCH（CS-RNTI 加扰）进一步指示
# CS-RNTI（Configured Scheduling RNTI）用于 SPS 激活/去激活 DCI
```

#### 16.2.3 多 SPS 配置（🔒 Rel-16 标准化）

Rel-16 引入多 SPS 配置支持，允许 UE 同时配置最多 8 个 SPS configuration（`sps-ConfigToAddModList`），各自对应不同 QoS Flow。
...
🔒 **Rel-16 已标准化** — TS 38.331 Clause 8.3（`PUSCH-Config.repetitionType`）定义了 PUSCH Repetition，分 Type A（连续 slot）和 Type B（mini-slot 跨 slot）。与 Slot Aggregation 不同，PUSCH Repetition 每次传输为 fresh encoding（独立 LDPC 编码），提供编码冗余而非时间冗余。
...
#### 16.3.1 Type A：基于 Slot 的连续重复（🔒 Rel-16）
...
#### 16.3.2 Type B：基于 Mini-slot 的重复（🔒 Rel-16）
...
#### 16.3.3 与 Slot Aggregation 的区别（🔒 Rel-16 已标准化）
...
🔒 **Rel-16 已标准化** — TS 23.501 Clause 5.7.7 定义了 5G QoS Monitoring 机制，gNB/UPF 分别测量 DL/UL 延迟并上报 SMF，用于 SLA 评估和 URLLC QoS 保障。
...
🔭 **Rel-18 方向性内容** — 以下特性在 3GPP Rel-18 标准化中讨论，部分特性可能在后续版本中调整，以 TS 38.300 Clause 9.2.8 为主要参考。

```
Rel-18 URLLC 增强方向（TS 38.300 Clause 9.2.8）：

1. AI/ML 辅助的 URLLC 调度（🔭 Rel-18 方向性）：
   ┌────────────────────────────────────────────────────────────────┐
   │  基于 AI 的预测性调度：                                          │
